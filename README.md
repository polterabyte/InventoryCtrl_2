...
- **[Уведомления](docs/NOTIFICATION_SYSTEM.md)** — система уведомлений в реальном времени (SignalR)
...
- Blazor WASM использует C# клиент `Microsoft.AspNetCore.SignalR.Client`; подключения к хабу выполняются внутри клиента без JS-бандлов.
- Клиентские URL (в т.ч. SignalR) формируются из конфигурации и относительных путей (`/api`, `/notificationHub`). В `src/Inventory.Web.Client/appsettings.{Environment}.json` используется секция `ApiSettings` с относительными путями.
- Файлы `appsettings*.json` клиента копируются в `wwwroot` при сборке/публикации и подхватываются в браузере.
- Общие статические ресурсы подключаются из RCL `Inventory.Web.Assets` по пути `_content/Inventory.Web.Assets/...` (например, `_content/Inventory.Web.Assets/js/push-notifications.js`).
