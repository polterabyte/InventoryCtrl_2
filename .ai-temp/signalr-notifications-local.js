// SignalR Notification Service - Local Version (Alternative)
class SignalRNotificationService {
    ...
    // Load SignalR library from local file
    async loadSignalRLibrary() {
        ...
        try {
            ...
            console.log('Loading SignalR from local file...');
            ...
            return new Promise((resolve, reject) => {
                script.onload = () => {
                    console.log('SignalR library loaded from local file');
                    this.signalRLoaded = true;
                    resolve(true);
                };
                script.onerror = (error) => {
                    console.error('Failed to load SignalR from local file:', error);
                    // Fallback to CDN
                    this.loadSignalRFromCDN().then(resolve).catch(reject);
                };
    ...
    // Fallback to CDN
    async loadSignalRFromCDN() {
        try {
            console.log('Loading SignalR from CDN as fallback...');
            const script = document.createElement('script');
            ...
            console.error('Error loading SignalR from CDN:', error);
            return false;
        }
    }
