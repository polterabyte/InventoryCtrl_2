...
### Blazor компоненты
- **Переиспользование**: Компоненты должны быть в Inventory.UI для переиспользования
- **Стили**: Использовать CSS модули app.css и design-system.css с логическими подфайлами компонентов (buttons.css, cards.css, forms.css, notifications.css). Максимально переиспользовать созданные стили
...
### Файлы Design System
- **Основной файл**: `src/Inventory.Shared/wwwroot/css/design-system.css`
- **Компоненты**: `src/Inventory.Shared/wwwroot/css/components/`
- **Темы**: `src/Inventory.Shared/wwwroot/css/themes/`
- **Порядок импорта**: Bootstrap → Design System → App Styles → Component Styles

### Единая CSS архитектура
Проект использует **единую CSS архитектуру**, где все стили централизованы в Shared проекте:
- **Единый источник истины**: Все CSS файлы находятся в `Inventory.Shared/wwwroot/css/`
- **Нет дублирования**: Web и будущие клиенты ссылаются на одни и те же файлы
- **Модульная структура**: Компоненты разделены на отдельные CSS файлы
- **Поддержка тем**: Светлые/темные темы с простой настройкой
- **Готовность к будущему**: Готовность для Mobile (MAUI) и Desktop (Electron/WPF) клиентов
